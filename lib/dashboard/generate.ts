...
type AppDemoParams = GenerationCallbacks & {
  screenshots: string[];
  description: string;
  demoFormat: "desktop" | "mobile";
  duration: string;
  quality: QualityMode;
  selectedVoiceId: string;
  musicEnabled: boolean;
...
export async function generateFromAppDemo(params: AppDemoParams) {
  const {
    screenshots,
    description,
    demoFormat,
  ...
  if (!screenshots.length || !description.trim()) return;
  ...
  cb.setProgress(5);
  cb.setStatus("scripting");
  cb.setFormatDetected("App Demo");

  try {
    const durationSeconds = durationToSeconds(duration);

    cb.setProgress(15);
    const analyzeRes = await fetch("/api/app-demo/analyze", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        screenshots,
        ...
        accentColor: accentColorLabel,
        focus,
      }),
    });
    const appData = await analyzeRes.json();
    if (!analyzeRes.ok) {
      cb.setError(appData.error || "Erreur d'analyse des screenshots");
      cb.setScreen("input");
      return;
    }
    ...
    let frameCursor = 0;
    const screenDurations = screens.map((s: { duration?: number }) => {
    ...
    const totalFrames = appData.totalFrames || frameCursor || 1800;

    cb.setProgress(35);
    cb.setStatus("voice");

    const voiceScript = screens
    ...
    if (voiceScript.trim()) {
      const voiceRes = await fetch("/api/voice", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ text: voiceScript, voiceId: selectedVoiceId }),
      });
      const voiceData = await voiceRes.json();
      if (voiceRes.ok) {
    ...
    const accent = appData.accent || appData.primaryColor || "#10B981";

    const renderRes = await fetch("/api/render", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        compositionId: "AppDemo",
        screens,
        screenDurations,
        totalFrames,
        format: demoFormat,
        ...
        musicVolume: 0.05,
        accent,
        appName: appData.appName || "App",
        bgColor: appData.bgColor || "#f5f5f7",
        prompt: description,
